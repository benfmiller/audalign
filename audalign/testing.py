    ...
    def create_plots(self, name, results, results_folder):
        for sec in range(0, len(self.test_seconds)):
            ...
            fig_name = os.path.join(
                results_folder, "%s_%s.png" % (name, self.test_seconds[sec])
            )
    ...
    def begin(self):
        for f in self.test_files:
            ...
            result = subprocess.check_output(
                ["python", "audalign.py", "-r", "file", self.test_folder + "/" + f]
            )
            ...
            if result.strip() == "None":
                ...
                if file_result != file:
                    ...
                    log_msg(
                        "query duration: %s" % round(result[audalign.MATCH_TIME], 3)
                    )
